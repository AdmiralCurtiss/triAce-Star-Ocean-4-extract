...
int main (int argc, char **argv) {
  ...
  // open the ISO
  if ((fp = fopen("soz0.bin", "rb")) == NULL) STOP("could not open soz0.bin");
  if ((fp2 = fopen("soz1.bin", "rb")) == NULL) STOP("could not open soz1.bin");
  ...
  for (i = 0; i < max * 3; i++) {
      table[i] = _byteswap_ulong(table[i]);
  }
  ...
  // make folder
  if (mkdir("soz") > 0) STOP("Error creando la carpeta");

  // extract each file
  for (i = 1; i < max; i++) {
    pos = table[max*1+i]; // LBA
    len = table[max*0+i]; // sectors
    U32 unknown = table[max*2+i];
    ...
    if (len) {
      ...
      if (fseeko64(currentfile, pos, SEEK_SET)) STOP("Error posicionando la imagen");
      if (fread(buffer, 1, len, currentfile) != len) STOP("Error leyendo la imagen");
      ...
      if (!hdr) {
        ...
        if (!tmp1 && (tmp2 == HEADER_010)) {
          sprintf(fn, "%s/%04d%s", "soz", i, EXTENSION_010);
          hdr = 0xFFFFFFFF;
        } else if (!tmp1 && (tmp2 == HEADER_000)) {
          sprintf(fn, "%s/%04d%s", "soz", i, EXTENSION_000);
          hdr = 0xFFFFFFFF;
        } else if (16 * (tmp1 + 1) == tmp2) {
          tmp3 = *(U32 *)(buffer + 0x1C);
          if (tmp3 == tmp2) {
            sprintf(fn, "%s/%04d%s", "soz", i, EXTENSION_PK1);
            hdr = 0xFFFFFFFF;
          }
        }
      } else if (hdr == HEADER_020) {
        sprintf(fn, "%s/%04d%s", "soz", i, EXTENSION_020);
        hdr = 0xFFFFFFFF;
      } else if (hdr < 0x100) {
        ...
        if ((tmp1 + tmp2) * SECTOR_LEN == len) {
          sprintf(fn, "%s/%04d%s", "soz", i, EXTENSION_PK2);
          hdr = 0xFFFFFFFF;
        }
      } else {
        tmp1 = *(U32 *)(buffer + 0x14);
        if (tmp1 == len) {
          sprintf(fn, "%s/%04d%s", "soz", i, EXTENSION_PK3);
          hdr = 0xFFFFFFFF;
        }
      }
      ...
      if (hdr != 0xFFFFFFFF) {
        switch (hdr) {
          case HEADER_ELF : sprintf(fn, "%s/%04d%s", "soz", i, EXTENSION_ELF); break;
          case HEADER_SLZ0:
          case HEADER_SLZ1:
          case HEADER_SLZ2:
          case HEADER_SLZ3: sprintf(fn, "%s/%04d%s", "soz", i, EXTENSION_SLZ); break;
          case HEADER_SLE0:
          case HEADER_SLE1:
          case HEADER_SLE2:
          case HEADER_SLE3: sprintf(fn, "%s/%04d%s", "soz", i, EXTENSION_SLE); break;
          case HEADER_ZLS : sprintf(fn, "%s/%04d%s", "soz", i, EXTENSION_ZLS); break;
          case HEADER_SEQ : sprintf(fn, "%s/%04d%s", "soz", i, EXTENSION_SEQ); break;
          case HEADER_PAC : sprintf(fn, "%s/%04d%s", "soz", i, EXTENSION_PAC); break;
          case HEADER_TXT : sprintf(fn, "%s/%04d%s", "soz", i, EXTENSION_TXT); break;
          case HEADER_DMY : sprintf(fn, "%s/%04d%s", "soz", i, EXTENSION_DMY); break;
          case HEADER_MC  :
          case HEADER_MC3 : sprintf(fn, "%s/%04d%s", "soz", i, EXTENSION_MC);  break;
          case HEADER_IDX :
          case HEADER_IDX3: sprintf(fn, "%s/%04d%s", "soz", i, EXTENSION_IDX); break;
          case HEADER_UNK : sprintf(fn, "%s/%04d%s", "soz", i, EXTENSION_UNK); break;
          case HEADER_KOD : sprintf(fn, "%s/%04d%s", "soz", i, EXTENSION_KOD); break;
          case HEADER_RCP : sprintf(fn, "%s/%04d%s", "soz", i, EXTENSION_RCP); break;
          case HEADER_FIS : sprintf(fn, "%s/%04d%s", "soz", i, EXTENSION_FIS); break;
          case HEADER_CRC : sprintf(fn, "%s/%04d%s", "soz", i, EXTENSION_CRC); break;
          default         : sprintf(fn, "%s/%04d%s", "soz", i, EXTENSION_BIN); break;
        }
      }

      // show the info
      printf("pos=%010I64X  len=%08X  ???=%08X ->  ", posorig, len, unknown);
      printf("%s\n", fn);
  ...
  if (fclose(fp2) == EOF) STOP("Error cerrando la iso");
